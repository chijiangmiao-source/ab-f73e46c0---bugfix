...
- `client_op_id` 是**全局**身份：一个标识永远只代表首次提交的操作。即使重试时切换了场次，
  同一标识携带不同内容（含不同场次）一律 **409**，并返回首次提交的场次与备注；
  新场次不会新增记录、计数器不推进，换用全新标识提交仍从 1 开始；
- 同一 `client_op_id` 携带**不同内容** → **409**；
- 不同操作并发成功后所得号码集合**无重复、无缺口**，号码顺序即数据库事务提交顺序；
- 操作映射与场次计数**只存于数据库**，不增加任何 worker，进程重启后据此恢复；
  若数据库中已存在历史版本遗留的跨场次重复映射，服务启动时会以**最早持久提交**为权威
  自愈：清除重复行、校正计数器（不留号码缺口、不回退权威场次计数）。
...
BEGIN IMMEDIATE                       -- 事务开始即取得写锁，发放事务彼此严格串行
  1. SELECT operations WHERE client_op_id = ?   -- 全局查询，不按场次收窄
       命中且内容哈希一致 → COMMIT，重放原号码（200）
       命中但内容哈希不同 → ROLLBACK，409（含切换场次重提，返回首次提交场次/备注）
...
- **兜底约束**：`operations.client_op_id` 全局唯一约束（跨场次也不可能二次占号）、
  `(scene_id, shot_number)` 唯一约束，由数据库最终保证不重号。
...
旧版本的唯一约束作用于 `(scene_id, client_op_id)`，故障注入后若终端误切场次重提，
可能在数据库中留下同一 `client_op_id` 的跨场次重复映射。服务启动时会检测并修复：

- 同组重复行只保留 **rowid 最小（最早持久提交）** 的一条 —— 首次提交为权威；
- 重建表以启用 `client_op_id` 全局唯一约束；
- 按剩余记录校正 `scene_counters`：权威场次计数为 `max(shot_number)+1`（不回退）；
  记录全部来自重复映射的场次移除计数器，下次发号从 1 开始，**不留号码缺口**。
