...
- 同一 `client_op_id` **全局唯一**：它永远代表首次提交的操作，与场次无关——
  相同内容重试（无论并发、重试还是进程重启）都返回**最初分配的号码**（幂等重放）；
- 同一 `client_op_id` 携带**不同内容**（含不同场次）→ **409**，响应指明首次提交的
  场次、备注与镜号；新场次不会因此新增记录或推进计数；
...
1. 冒烟检查 `api` / `web` 可达；
2. **pytest**：20 并发无重复无缺口、并发重复重试共享号码、跨场次争用同一标识只有一个首次成功、
   进程重启后映射与计数恢复、注入故障只触发一次、409 冲突、异常数据启动恢复、参数校验
   （另含对 compose 服务的端到端冒烟）；
...
BEGIN IMMEDIATE                       -- 事务开始即取得写锁，发放事务彼此严格串行
  1. SELECT operations WHERE client_op_id = ?      -- 全局查找，不限场次
       命中且内容哈希一致 → COMMIT，重放原号码（200）
       命中但内容哈希不同 → ROLLBACK，409（响应携带首次提交的场次/备注/镜号）
...
- **兜底约束**：`operations.client_op_id` 全局唯一约束、`(scene_id, shot_number)` 唯一约束，
  由数据库最终保证同一标识不二次占号、同场次不重号。
- 读接口（列表/单查/健康检查）在事务外自动提交模式下执行，不参与发号串行化。

## 异常数据恢复

若数据库由旧版缺陷服务留下**跨场次重复映射**（同一 `client_op_id` 在不同场次各占一号），
服务启动迁移时以**首次持久提交**（`id` 最小）为权威保留该标识；其余重复行不删除——其号码
可能已被现场知悉——而是改用 `recovered-op-<id>` 重新标识。各场次号码序列与计数器原样保留：
不留号码缺口，也不回退计数。迁移幂等，重启后结论不变。
